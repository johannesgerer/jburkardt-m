function fem2d_navier_stokes_sparse ( prefix )

%*****************************************************************************80
%
%% MAIN is the main routine of FEM2D_NAVIER_STOKES_SPARSE.
%
%  Discussion:
%
%    This program is a variant of FEM2D_NAVIER_STOKES.  That program is
...
%
%    A few routines needed to be changed:
%    * the main program FEM2D_NAVIER_STOKES is replaced by FEM2D_NAVIER_STOKES_SPARSE.
...
%    * the routine JACOBIAN_FEM is replaced by JACOBIAN_FEM_SPARSE.
%    * the routine JACOBIAN_ADJUST_DIRICHLET is replaced by
%      JACOBIAN_ADJUST_DIRICHLET_SPARSE.
%
...
%      nu = constants ( )
%
%    * the right hand sides of the Navier-Stokes equations
%      by supplying a routine of the form
%
%      function [ u_rhs, v_rhs, p_rhs ] = rhs ( xy )
%
%    * the right hand sides of the Dirichlet boundary
%      conditions by supplying a function
%
%      function [ u_bc, v_bc, p_bc ] = dirichlet_condition ( xy )
%
%  Usage:
%
%    fem2d_navier_stokes_sparse ( 'prefix' )
%
%    where:
%
...
%  Local parameters:
%
%    Local, real sparse A, the Stokes stiffness matrix or Navier Stokes
%    jacobian matrix, stored as a MATLAB sparse matrix.
%
...
%
  ...
  fprintf ( 1, '\n' );
  fprintf ( 1, 'FEM2D_NAVIER_STOKES_SPARSE\n' );
...
%  the nonzeros.
%
  element_neighbor = triangulation_order6_neighbor_triangles ( ...
    element_num, element_node );
%
...
%
  ...
  while ( 1 )
    ...
    a = jacobian_fem_sparse ( node_num, node_xy, element_num, ...
      element_node, quad_num, node_u_variable, ...
      node_v_variable, node_p_variable, variable_num, nu, node_c, nz_num );
%
%  Adjust the jacobian for boundary conditions.
%
    a = jacobian_adjust_dirichlet_sparse ( node_num, node_xy, ...
...
function a = jacobian_fem_sparse ( node_num, node_xy, element_num, ...
  element_node, quad_num, node_u_variable, node_v_variable, ...
  node_p_variable, variable_num, nu, c, nz_num )

%*****************************************************************************80
%
%% JACOBIAN_FEM_SPARSE evaluates the Navier Stokes jacobian.
%
%  Discussion:
%
%    The matrix is set up as a MATLAB sparse matrix.
%
...
%    to take over the task of reallocating space as necessary.
%
%    Output, real sparse A, the Navier Stokes jacobian, stored
%    as a MATLAB sparse matrix.
%
  fprintf ( 1, '\n' );
  fprintf ( 1, 'JACOBIAN_FEM_SPARSE:\n' );
  fprintf ( 1, '  Setting up sparse Stokes matrix with NZ_NUM = %d\n', nz_num );
%
...
%
    for i = 1 : 6
      for j = 1 : 6
        ...
        a(iu(i),iv(j)) = a(iu(i),iv(j)) + sum ...
        ( w(1:quad_num) .* b(j,1:quad_num) .* dudy(1:quad_num) ...
        .* b(i,1:quad_num) );
      end

      for j = 1 : 3
        a(iu(i),ip(j)) = a(iu(i),ip(j)) + sum ...
...
%
    for i = 1 : 6
      for j = 1 : 6
        ...
        a(iv(i),iu(j)) = a(iv(i),iu(j)) + sum ...
        ( w(1:quad_num) .* b(j,1:quad_num) .* dvdx(1:quad_num)            ...
          .* b(i,1:quad_num) );

        a(iv(i),iv(j)) = a(iv(i),iv(j)) + sum ...
        ( w(1:quad_num) .*                                               ...
          (                                                             ...
            nu * ( dbdx(j,1:quad_num) .* dbdx(i,1:quad_num)              ...
                 + dbdy(j,1:quad_num) .* dbdy(i,1:quad_num) )            ...
...
%
    for i = 1 : 3
      for j = 1 : 6
        ...
        a(ip(i),iu(j)) = a(ip(i),iu(j)) + sum ...
        ( w(1:quad_num) .* dbdx(j,1:quad_num) .* q(i,1:quad_num) );

        a(ip(i),iv(j)) = a(ip(i),iv(j)) + sum ...
