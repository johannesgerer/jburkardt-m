function value = p27_r8vec ( action, name, dim_num, value )
...
%
%    Input/output, real VALUE(DIM_NUM,1), the value of the object.
%
  persistent c
  ...
  if ( action == 'D' | action == 'd' )
    ...
    if ( name == 'C' | name == 'c' | name == '*' )
      c(1:dim_num,1) = 1.0 / dim_num;
    end

  elseif ( action == 'G' | action == 'g' )
      
    if ( name == 'C' | name == 'c' )
      value = zeros ( 1:dim_num, 1 );
      value(1:dim_num,1) = c(1:dim_num,1);
    else
      fprintf ( 1, '\n' );
      fprintf ( 1, 'P27_R8VEC - Fatal error!\n' );
      fprintf ( 1, '  Unrecognized name = "%s".\n', name );
      error ( 'P27_R8VEC - Fatal error!' );
    end
  ...
  elseif ( action == 'R' | action == 'r' )
    ...
    if ( name == 'C' | name == 'c' )
      c = rand ( 1, dim_num );
      c(1:dim_num,1) = c(1:dim_num,1) * 25.0 / dim_num;
      value(1:dim_num,1) = c(1:dim_num,1);
    else
      fprintf ( 1, '\n' );
      fprintf ( 1, 'P27_R8VEC - Fatal error!\n' );
  ...
  elseif ( action == 'S' | action == 's' )
    ...
    if ( name == 'C' | name == 'c' )
      c(1:dim_num,1) = value(1:dim_num,1);
    else
      fprintf ( 1, '\n' );
      fprintf ( 1, 'P27_R8VEC - Fatal error!\n' );
      fprintf ( 1, '  Unrecognized name = "%s".\n', name );
