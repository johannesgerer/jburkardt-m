function [ ap, det ] = sppdi ( ap, n, job )
...
%
  if ( mod ( job, 10 ) ~= 0 )
    ...
    for k = 1 : n
      ...
      ap(kk) = 1.0 / ap(kk);
      t = -ap(kk);
      ap(k1:k1+k-2) = sscal ( k-1, t, ap(k1:k1+k-2), 1 );
      j1 = kk + 1;
      kj = kk + k;
      ...
      for j = k + 1 : n
        t = ap(kj);
        ap(kj) = 0.0;
        ap(j1:j1+k-1) = saxpy ( k, t, ap(k1:k1+k-1), 1, ap(j1:j1+k-1), 1 );
        j1 = j1 + j;
        kj = kj + j;
      end
...
%
    ...
    for j = 1 : n
      ...
      for k = 1 : j-1
        t = ap(kj);
        ap(k1:k1+k-1) = saxpy ( k, t, ap(j1:j1+k-1), 1, ap(k1:k1+k-1), 1 );
        k1 = k1 + k;
        kj = kj + 1;
      end

      t = ap(jj);
      ap(j1:j1+j-1) = sscal ( j, t, ap(j1:j1+j-1), 1 );
