function vector_magnitude_grid ( node_file, vector_file, grid_num, contour_num )

%*****************************************************************************80
%
%% VECTOR_MAGNITUDE_GRID contour plots a vector magnitude from scattered data.
%
%  Discussion:
%
...
%  Usage:
%
%    vector_magnitude_grid ( node_file, vector_file, contour_num )
%
%  Parameters:
%
...
%
%    Input, integer CONTOUR_NUM, the number of contour lines to use.
%
  fprintf ( 1, '\n' );
  timestamp ( );
...
%
%  Fourth argument is the number of contour lines.
%
  if ( nargin < 4 )
    fprintf ( 1, '\n' );
    contour_num = input ( '  Enter the number of contour lines.' );
  end
%
%  Read the node data.
%
  ...
  if ( dim_num ~= 2 )
    ...
    error ( 'VECTOR_MAGNITUDE_GRID - Fatal error!' );
  end
...
%
  [ vector_order, node_num2 ] = r8mat_header_read ( vector_file );
  ...
  uv = r8mat_data_read ( vector_file, vector_order, node_num );
...
%
  uv_norm_grid(:,:) = sqrt ( u_grid(:,:).^2 + v_grid(:,:).^2 );
%
%  Make a vector magnitude contour plot using the grid data.
%
  contour ( x_grid, y_grid, uv_norm_grid, contour_num )
  ...
  title ( 'Vector Magnitudes', 'FontName', 'Helvetica', 'FontWeight', 'bold', ...
    'FontSize', 16 );
