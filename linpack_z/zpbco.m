function [ abd, rcond, info ] = zpbco ( abd, lda, n, m )
...
%
  for j = 1 : n
    ...
    l = min ( j, m + 1 );
    mu = max ( m + 2 - j, 1 );
    z(j) = dzasum ( l, abd(mu:mu+l-1,j), 1 );
    k = j - l;

    for i = mu : m
      k = k + 1;
      z(k) = real ( z(k) ) + zabs1 ( abd(i,j) );
    end
...
%
  ...
  for k = 1 : n
    ...
    if ( real ( abd(m+1,k) ) < zabs1 ( ek - z(k) ) )
      s = real ( abd(m+1,k) ) / zabs1 ( ek - z(k) );
      z(1:n) = z(1:n) * s;
      ek = s * ek;
    end
    ...
    if ( k+1 <= j2 )
      ...
      for j = k+1 : j2
        i = i - 1;
        sm = sm + zabs1 ( z(j) + wkm * conj ( abd(i,j) ) );
...
%
  for k = n : -1 : 1
    ...
    if ( real ( abd(m+1,k) ) < zabs1 ( z(k) ) )
      s = real ( abd(m+1,k) ) / zabs1 ( z(k) );
      z(1:n) = z(1:n) * s;
    end
