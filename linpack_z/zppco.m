function [ ap, rcond, info ] = zppco ( ap, n )
...
%
  ...
  for k = 1 : n
    ...
    if ( real ( ap(kk) ) < zabs1 ( ek - z(k) ) )
      s = real ( ap(kk) ) / zabs1 ( ek - z(k) );
      z(1:n) = z(1:n) * s;
      ek = s * ek;
    end
    ...
    if ( k+1 <= n )
      ...
      for j = k+1 : n
        sm = sm + zabs1 ( z(j) + wkm * conj ( ap(kj) ) );
...
%
  for k = n : -1 : 1
    ...
    if ( real ( ap(kk) ) < zabs1 ( z(k) ) )
      s = real ( ap(kk) ) / zabs1 ( z(k) );
      z(1:n) = z(1:n) * s;
    end
