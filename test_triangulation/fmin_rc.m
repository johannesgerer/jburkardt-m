...
%  Parameters
%
%    Input, real A, B, the current estimates for the left and right 
%    endpoints of the interval.  It is required that A < B.
%
%    Input, integer STATUS, set to zero by the user on the first call only.
%    Thereafter, the input value of STATUS should be the output value from
%    the previous call.
%
%    Input, real VALUE, the function value at ARG.  On the very first call,
%    VALUE is not needed, and may be set to zero.
%
%    Output, real A, B, a smaller interval containing the minimizer.  
%
%    Output, real ARG, the currently considered point.  On return with 
...
%
%    Output, integer STATUS, used to communicate between the user
