function fem2d_stokes ( prefix )

%*****************************************************************************80
%
%% MAIN is the main routine of FEM2D_STOKES.
%
%  Discussion:
%
...
%    finite element coefficient matrix, stored in a banded format.
%
...
%
  if ( node_num <= 100 )
    ...
    filename = 'fem2d_stokes_nodes.eps';
    node_label = 1;
...
%
  if ( node_num <= 100 )
    ...
    filename = 'fem2d_stokes_elements.eps';
    node_show = 2;
    triangle_show = 2;
...
%
  ...
  if ( debugging )
    ...
    r8vec_print_some ( variable_num, f, 1, 10, ...
      '  Part of right hand side after Dirichlet BC adjustment:' );

  end
%
%  Adjust the linear system to account for Neumann boundary conditions.
%
  f = neumann_apply ( node_num, node_xy, node_p_variable, ...
    node_u_variable, node_v_variable, node_p_condition, ...
  ...
  if ( 0 )
    ...
    r8vec_print_some ( variable_num, f, 1, 10, ...
      '  Part of right hand side after Neumann BC adjustment:' );
...
function ib = bandwidth ( element_num, element_node, ...
  node_num, node_p_variable, node_u_variable, node_v_variable )
...
%    Input, integer ELEMENT_NUM, the number of elements.
%
%    Input, integer ELEMENT_NODE(6,ELEMENT_NUM);
...
%*****************************************************************************80
%
%% BASIS_MN_T3 evaluates all basis functions for a linear triangle.
%
%  Discussion:
%
...
function f = neumann_apply ( node_num, node_xy, node_p_variable, ...
  node_u_variable, node_v_variable, node_p_condition, ...
  node_u_condition, node_v_condition, variable_num, f )
...
%  Parameters:
%
%    Input, string FILE_NAME, the file name.
%
%    Input, integer NODE_NUM, the number of nodes.
%
%    Input, integer NODE_P_VARIABLE(NODE_NUM),
