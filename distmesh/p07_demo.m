function p07_demo ( iteration_max, h )

%*****************************************************************************80
%
%% P07_DEMO runs the 2D demo problem #7, with mesh size H.
%
%  Discussion:
%
...
%
  ...
  fprintf ( 1, '\n' );
  fprintf ( 1, 'Problem 7:\n' );
  fprintf ( 1, '  The bicycle seat, h = %f\n', h )

  fd = @p07_fd;
  fh = @p07_fh;
  box = [-5*pi/2,-5.0; 5*pi/2, 2.0 ];
  fixed = 5*pi/2 * [-1.0,0.0; 1.0,0.0];
...
%
  ...
  triangulation_order3_plot ( 'p07_mesh.eps', node_num, p, tri_num, ...
    t, node_show, triangle_show );
%
%  Write a text file containing the nodes.
%
  r8mat_write ( 'p07_nodes.txt', 2, node_num, p );
%
%  Write a text file containing the triangles.
%
  i4mat_write ( 'p07_elements.txt', 3, tri_num, t );
