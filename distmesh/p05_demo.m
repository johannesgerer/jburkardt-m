function p05_demo ( iteration_max, h )

%*****************************************************************************80
%
%% P05_DEMO runs the 2D demo problem #5, with mesh size H.
%
%  Licensing:
%
...
%
  ...
  fprintf ( 1, '\n' );
  fprintf ( 1, 'Problem 5:\n' );
  fprintf ( 1, '  The horn, h = %f\n', h )

  fd = @p05_fd;
  fh = @p05_fh;
  box = [-1.0,0.0; 1.0,1.0];
  fixed = [ -1.0, 0.0; -0.95, 0.0; 0.15, 0.0; 1.0, 0.0 ];
...
%
  ...
  triangulation_order3_plot ( 'p05_mesh.eps', node_num, p, tri_num, ...
    t, node_show, triangle_show );
%
%  Write a text file containing the nodes.
%
  r8mat_write ( 'p05_nodes.txt', 2, node_num, p );
%
%  Write a text file containing the triangles.
%
  i4mat_write ( 'p05_elements.txt', 3, tri_num, t );
