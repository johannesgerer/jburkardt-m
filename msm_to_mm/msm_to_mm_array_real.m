function msm_to_mm_array_real ( output_filename, a, symmetry ) 

%*****************************************************************************80
%
%% MSM_TO_MM_ARRAY_REAL writes a "matrix array real" Matrix Market file.
%
%  Licensing:
%
...
%
  if ( s_eqi ( symmetry, 'GENERAL' ) )
    msm_to_mm_array_real_general ( output_filename, a );
  elseif ( s_eqi ( symmetry, 'SYMMETRIC' ) )
    msm_to_mm_array_real_symmetric ( output_filename, a );
  elseif ( s_eqi ( symmetry, 'SKEW-SYMMETRIC' ) )
    msm_to_mm_array_real_skewsymmetric ( output_filename, a );
  else
    fprintf ( 1, '\n' );
    fprintf ( 1, 'MSM_TO_MM_ARRAY_REAL - Fatal error!\n' );
    fprintf ( 1, '  Illegal symmetry option.\n' );
    error ( 'MSM_TO_MM_ARRAY_REAL - Fatal error!' );
  end
