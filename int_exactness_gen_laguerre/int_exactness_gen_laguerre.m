function int_exactness_gen_laguerre ( quad_filename, degree_max, alpha, option )
...
%  Discussion:
%
%    This program investigates a geeralized Gauss-Laguerre quadrature rule
...
%
  ...
  fprintf ( 1, '  monomials up to a given degree over the [0,+oo) interval.\n' );
  fprintf ( 1, '\n' );
  fprintf ( 1, '  The rule may be defined on another interval, [A,+oo)\n' );
  fprintf ( 1, '  in which case it is adjusted to the [0,+oo) interval.\n' );
%
...
%
  if ( 4 <= nargin )
    ...
    fprintf ( 1, '  0: standard rule for integrating x^alpha*exp(-x)*f(x);\n' );
...
%
  fprintf ( 1, '\n' );
  fprintf ( 1, 'INT_EXACTNESS_GEN_LAGUERRE: User input:\n' );
  ...
  if ( option == 0 )
    fprintf ( 1, '  OPTION = 0, integrate x^alpha*exp(-x)*f(x).\n' );
  else
...
%
  ...
  fprintf ( 1, '  a generalized Gauss-Laguerre rule\n' );
  fprintf ( 1, '  ORDER = %d\n', order );
  fprintf ( 1, '  A =     %f\n', a );
  fprintf ( 1, '  ALPHA = %f\n', alpha );
  fprintf ( 1, '\n' );
...
%
  fprintf ( 1, '\n' );
  fprintf ( 1, '  A generalized Gauss-Laguerre rule would be able to exactly\n' );
...
%
  exact = r8_gamma ( alpha + expon + 1 );

  return
end
function quad_error = monomial_quadrature_gen_laguerre ( expon, alpha, ...
  order, option, w, x )
...
%
  if ( option == 0 )
    ...
    value(1:order) = x(1:order).^alpha .* exp ( - x(1:order) ) .* x(1:order).^expon;
  end
%
