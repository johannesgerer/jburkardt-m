function vandermonde_interp_1d_test01 ( prob )

%*****************************************************************************80
%
%% VANDERMONDE_INTERP_1D_TEST01 tests VANDERMONDE_INTERP_1D_MATRIX.
%
%  Licensing:
%
...
%
  fprintf ( 1, '\n' );
  fprintf ( 1, 'VANDERMONDE_INTERP_1D_TEST01:\n' );
...
%
  ...
  condition = cond ( A, 2 );
  fprintf ( 1, '\n' );
  fprintf ( 1, '  Condition of Vandermonde matrix is %g\n', condition );
%
%  Solve linear system.
%
  c = A \ yd;
...
%
  ...
  xlabel ( '<--- X --->' );
  ylabel ( '<--- Y --->' );
  title ( sprintf ( 'Vandermonde Polynomial Interpolant of degree %d', m ) )
  grid on
  hold off
