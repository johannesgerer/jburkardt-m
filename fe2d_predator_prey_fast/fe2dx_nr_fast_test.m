function fe2dx_nr_fast_test ( )

%*****************************************************************************80
%
%% FE2DX_NR_FAST_TEST tests the FE2DX_NR_FAST code.
%
%  Discussion:
%
%    This function sets all parameter values and initial condition information
%    necessary to execute the "fast" version of the fe2dx_nr algorithm.
%
%  Licensing:
%
...
%
  timestamp ( );
  fprintf ( 1, '\n' );
  fprintf ( 1, 'FE2DX_NR_FAST_TEST:\n' );
  fprintf ( 1, '  Test the FE2DX_NR_FAST function, which\n' );
...
% T = 150.0;
  ...
  t = tic;
  fe2dx_nr_fast ( alpha, beta, gamma, delta, T, delt, @u0f, @v0f, k1, ...
    k2, @g2uf, @g2vf );
  t = toc ( t );
