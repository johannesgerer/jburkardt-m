function [ result, node_num ] = sphere01_quad_icos1m ( factor, fun )
...
%    be at least 1.
%
%    Input, function v = FUN ( x, v ), evaluates the integrand.
%
...
%
    for f1 = 0 : factor - 1
      for f2 = 0 : factor - f1 - 1
        ...
        a3_xyz = sphere01_triangle_project ( ...
          a_xyz, b_xyz, c_xyz, 2 * f1 + 1, 2 * f2 + 1, 2 * f3 - 2 );
        b3_xyz = sphere01_triangle_project ( ...
          a_xyz, b_xyz, c_xyz, 2 * f1,     2 * f2 + 1, 2 * f3 - 1 );
        c3_xyz = sphere01_triangle_project ( ...
