function node_xyz = sphere01_triangle_project2 ( a_xyz, b_xyz, c_xyz, f1, f2, f3 )
...
%
  if ( f2 == 0 && f3 == 0 )
    node_xyz(1:3,1) = a_xyz(1:3,1);
    return
  elseif ( f1 == 0 && f3 == 0 )
    node_xyz(1:3,1) = b_xyz(1:3,1);
    return
  elseif ( f1 == 0 && f2 == 0 )
    node_xyz(1:3,1) = c_xyz(1:3,1);
    return
   end
%
...
%
  ...
  ab(1:3,1) = cos ( angle ) * a_xyz(1:3,1) + sin ( angle ) * bn(1:3,1);

  angle = ( f2 + f3 ) * theta_ac / ( f1 + f2 + f3 );
  ac(1:3,1) = cos ( angle ) * a_xyz(1:3,1) + sin ( angle ) * cn(1:3,1);
%
%  Determine the angular distance between AB and AC.
%
  theta_bc = sphere01_distance_xyz ( ab(1:3,1), ac(1:3,1) );
%
...
%
  ...
  node_xyz(1:3,1) = cos ( angle ) * ab(1:3,1) + sin ( angle ) * acn(1:3,1);
