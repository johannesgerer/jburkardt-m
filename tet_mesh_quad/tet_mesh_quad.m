function [ quad, volume_sum ] = triangulation_quad ( prefix )
...
%    over a region defined by a tetrahedral mesh, using only the values of 
%    the function at the nodes of the tet mesh.
%
%  Usage:
%
%    [ quad, volume_sum ] = tet_mesh_quad ( 'prefix' )
%
%    where
%
...
%
%    volume_sum is the volume of the region covered by the tetrahedral mesh.
%
%  Licensing:
%
...
%    components of the function over the tet mesh.
%
%    Output, real VOLUME_SUM, the total volume of the tetrahedral mesh.
%
%  Local parameters:
%
...
%
%    Local, integer ELEMENT_ORDER, the order of the elements, 
%    either 4 or 10.
%
%    Local, integer NODE_DIM, the spatial dimension.
%
%    Local, integer NODE_NUM, the number of nodes.
%
%    Local, real NODE_XYZ(NODE_DIM,NODE_NUM), the point set.
%
%    Local, real VALUE(VALUE_DIM,VALUE_NUM), the VALUE_DIM components of a
%    vector function evaluated at each of the NODE_NUM nodes.
%
%    Local, integer VALUE_DIM, the number of components of the function.
%
%    Local, integer VALUE_NUM, the number of points at which function values
...
%
  ...
  if ( debug )
    ...
    fprintf ( 1, '\n' );
    fprintf ( 1, 'TET_MESH_QUAD:\n' );
    fprintf ( 1, '  MATLAB version:\n' );
    fprintf ( 1, '  Estimate an integral over a region defined bhy a tetrahedral mesh.\n' );
  end
%
...
%
  ...
  if ( node_dim ~= 3 )
    fprintf ( 1, '\n' );
    fprintf ( 1, 'TET_MESH_QUAD - Fatal error!\n' );
    fprintf ( 1, '  Dataset must have spatial dimension 3.\n' );
    error ( 'TET_MESH_QUAD - Fatal error!' );
  end

  node_xyz = r8mat_data_read ( node_filename, node_dim, node_num );

  if ( debug )
    fprintf ( 1, '\n' );
    fprintf ( 1, '  Read the data in "%s".\n', node_filename );

    r8mat_transpose_print_some ( node_dim, node_num, node_xyz, 1, 1, node_dim, ...
      5, '  First 5 nodes:' );
  end
%
...
%
  volume(1,1:element_num) = ( 1.0 / 6.0 ) * abs (                                               ...
      ( node_xyz(1,element_node(2,1:element_num)) - node_xyz(1,element_node(1,1:element_num)) )       ...
      .* ( ( node_xyz(2,element_node(3,1:element_num)) - node_xyz(2,element_node(1,1:element_num)) )   ...
        .* ( node_xyz(3,element_node(4,1:element_num)) - node_xyz(3,element_node(1,1:element_num)) )   ...
        - ( node_xyz(3,element_node(3,1:element_num)) - node_xyz(3,element_node(1,1:element_num)) )   ...
        .* ( node_xyz(2,element_node(4,1:element_num)) - node_xyz(2,element_node(1,1:element_num)) ) ) ...
    + ( node_xyz(2,element_node(2,1:element_num)) - node_xyz(2,element_node(1,1:element_num)) )       ...
      .* ( ( node_xyz(3,element_node(3,1:element_num)) - node_xyz(3,element_node(1,1:element_num)) )   ...
        .* ( node_xyz(1,element_node(4,1:element_num)) - node_xyz(1,element_node(1,1:element_num)) )   ...
        - ( node_xyz(1,element_node(3,1:element_num)) - node_xyz(1,element_node(1,1:element_num)) )   ...
        .* ( node_xyz(3,element_node(4,1:element_num)) - node_xyz(3,element_node(1,1:element_num)) ) ) ...
    + ( node_xyz(3,element_node(2,1:element_num)) - node_xyz(3,element_node(1,1:element_num)) )       ...
      .* ( ( node_xyz(1,element_node(3,1:element_num)) - node_xyz(1,element_node(1,1:element_num)) )   ...
        .* ( node_xyz(2,element_node(4,1:element_num)) - node_xyz(2,element_node(1,1:element_num)) )   ...
        - ( node_xyz(2,element_node(3,1:element_num)) - node_xyz(2,element_node(1,1:element_num)) )   ...
        .* ( node_xyz(1,element_node(4,1:element_num)) - node_xyz(1,element_node(1,1:element_num)) ) ) );
...
%
  quad(1:value_dim,1) = ...
    ...
    + value(1:value_dim,element_node(3,1:element_num)) ...
    + value(1:value_dim,element_node(4,1:element_num)) ) * volume(1,1:element_num)' / 4.0;

  volume_sum = sum ( volume(1,1:element_num) );
%
%  Terminate.
%
  if ( debug )
