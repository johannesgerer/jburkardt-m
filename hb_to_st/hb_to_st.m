function hb_to_st ( input_file, output_file )

%*****************************************************************************80
%
%% HB_TO_ST reads a Harwell-Boeing Sparse Matrix File, and writes an ST file.
%
%  Licensing:
%
...
%
%    Input, string OUTPUT_FILE, the name of the ST file to be created.
%
  input_unit = fopen ( input_file );

  if ( input_unit < 0 )
    fprintf ( 1, '\n' );
    fprintf ( 1, 'HB_TO_ST - Fatal error!\n' );
  ...
  base0 = 0;
  base1 = 1;
  rowind = st_rebase ( base1, base0, nnzero, rowind );
  colind = st_rebase ( base1, base0, nnzero, colind );

  st_header_print ( nrow, ncol, nnzero );

  st_header_write ( output_file, nrow, ncol, nnzero );

  st_data_print ( nrow, ncol, nnzero, rowind, colind, values, ...
    '  Data read from HB file' );

  st_data_write ( output_file, nrow, ncol, nnzero, rowind, colind, values );
...
function st_header_write ( output_filename, nrow, ncol, nnzero )
