function fem1d_heat_implicit_test02 ( )

%*****************************************************************************80
%
%% FEM1D_HEAT_IMPLICIT_TEST02 does a problem with known solution.
%
%  Licensing:
%
...
%
  fprintf ( 1, '\n' );
  fprintf ( 1, 'FEM1D_HEAT_IMPLICIT_TEST02:\n' );
...
%
  ...
  for j = 1 : t_num
    ...
    if ( j == 1 )
      ...
      h(1:x_num,1) = fem1d_heat_implicit ( x_num, x, t(j), dt, @k_test02, ...
        @rhs_test02, @bc_test02, element_num, element_node, quad_num, h );
    end
...
%
  ...
  title ( 'H(X,T) for TEST02 computed by FEM1D\_HEAT\_IMPLICIT' );
  xlabel ( '<-- Time -->' );
  ylabel ( '<-- X -->' );
  zlabel ( '<--  H(X,T) -->' );
%
