function fem1d_heat_implicit_test03 ( )

%*****************************************************************************80
%
%% FEM1D_HEAT_IMPLICIT_TEST03 does a simple test problem.
%
%  Licensing:
%
...
%
  fprintf ( 1, '\n' );
  fprintf ( 1, 'FEM1D_HEAT_IMPLICIT_TEST03:\n' );
...
%
  ...
  for j = 1 : t_num
    if ( j == 1 )
      ...
      h(1:x_num,1) = fem1d_heat_implicit ( x_num, x, t(j), dt, @k_test03, ...
        @rhs_test03, @bc_test03, element_num, element_node, quad_num, h );
    end
...
%
  ...
  title ( 'H(X,T) for TEST03 computed by FEM1D\_HEAT\_IMPLICIT' );
  xlabel ( '<-- Time -->' );
  ylabel ( '<-- X -->' );
  zlabel ( '<--  H(X,T) -->' );
%
