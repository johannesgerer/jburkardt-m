function fd1d_heat_implicit_test01 ( )
...
%
  fprintf ( 1, '\n' );
  fprintf ( 1, 'FD1D_HEAT_IMPLICIT_TEST01:\n' );
...
%
  cfl = fd1d_heat_implicit_cfl ( k, t_num, t_min, t_max, x_num, ...
    x_min, x_max );
...
%
  ...
  mesh ( tmat, xmat, hmat );
  title ( 'H(X,T) for TEST01 computed by FD1D\_HEAT\_IMPLICIT' );
  xlabel ( '<-- Time -->' );
  ylabel ( '<-- X -->' );
  zlabel ( '<--  H(X,T) -->' );
%
