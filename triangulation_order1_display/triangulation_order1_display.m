function triangulation_order1_display ( prefix )

%*****************************************************************************80
%
%% TRIANGULATION_ORDER1_DISPLAY plots piecewise constant triangulated data.
%
%  Discussion:
%
%    This program reads three data files defining piecewise constant
%    data over a triangulation, and displays a 3D MATLAB plot of the data.
%
%  Usage:
%
...
%    * 'prefix'_values.txt contains the values associated with each triangle.
%
%  Licensing:
%
...
%
  ...
  fprintf ( 1, '\n' );
  fprintf ( 1, 'TRIANGULATION_ORDER1_DISPLAY:\n' );
  fprintf ( 1, '  MATLAB version\n' );
  fprintf ( 1, '\n' );
  fprintf ( 1, '  Read and plot piecewise constant triangulated data.\n' );
  fprintf ( 1, '\n' );
...
%
  ...
  if ( dim_num ~= 2 )
    fprintf ( 1, '\n' );
    fprintf ( 1, 'TRIANGULATION_ORDER1_DISPLAY - Fatal error!\n' );
    fprintf ( 1, '  Dataset must have spatial dimension 2.\n' );
    error ( 'TRIANGULATION_ORDER1_DISPLAY - Fatal error!' );
  end
...
%
  ...
  trimesh ( t(:,1:3), node_xy(1,:), node_xy(2,:), 'Color', 'blue' );
  ...
  triangulation_order1_display_image ( node_xy, triangle_num, ...
    triangle_node, value );
...
function triangulation_order1_display_image ( node_xy, ...
  triangle_num, triangle_node, value )

%*****************************************************************************80
%
%% TRIANGULATION_ORDER1_DISPLAY_IMAGE plots piecewise constant data.
%
%  Licensing:
%
%    This code is distributed under the GNU LGPL license.
...
%    the nodes that made up each triangle.
%
%    Input, real VALUE(TRIANGLE_NUM), the value assigned to each triangle.
%
  zmax = max ( value(1:triangle_num) );
  zmin = min ( value(1:triangle_num) );
%
...
%
    x1 = node_xy(1,triangle_node(1,triangle));
    y1 = node_xy(2,triangle_node(1,triangle));
    x2 = node_xy(1,triangle_node(2,triangle));
    y2 = node_xy(2,triangle_node(2,triangle));
    x3 = node_xy(1,triangle_node(3,triangle));
    y3 = node_xy(2,triangle_node(3,triangle));
...
%
    ...
    fill3 ( [ x1, x2, x3 ], [ y1, y2, y3 ], [ z, z, z ], [ r, g, b ] )
%
