function matern_test ( )

%*****************************************************************************80
%
%% MATERN_TEST examines the matern correlation.
%
%  Licensing:
%
...
%
  F = fred ( @matern_correlation, domain ( [ a, b ] ) );
%
...
%
  ...
  title ( 'matern: Mercer eigenvalues' );
  xlabel ( '<--- N --->')
  grid on
  print -dpng 'matern_figure1.png'
%
...
%
  ...
  title ( 'matern: Mercer eigenfunction PSI(1)' )
  grid on
  subplot ( 4, 1, 2 )
  plot ( Psi(:,2), 'Linewidth', 2 );
  title ( 'matern: Mercer eigenfunction PSI(2)' )
  grid on
  subplot ( 4, 1, 3 )
  plot ( Psi(:,3), 'Linewidth', 2 );
  title ( 'matern: Mercer eigenfunction PSI(3)' )
  grid on
  if ( 4 <= eigen_num )
    subplot ( 4, 1, 4 )
    plot ( Psi(:,4), 'Linewidth', 2 );
    title ( 'matern: Mercer eigenfunction PSI(4)' )
    grid on
  end
  print -dpng 'matern_figure2.png'
%
...
%
  ...
  xlabel ( '<-- N -->' )
  title ( 'matern: Eigenvalue decay rate' );
  print -dpng 'matern_figure3.png'
%
...
%
  ...
  title ( 'matern: 40 Random Realizations X(t,omega), and their Mean.' )
  print -dpng 'matern_figure4.png'
%
...
%
  ...
  mesh ( S, T, C );
  hold on;
  D = matern_correlation ( S, T );
  plot3 ( S, T, D, 'k.', 'markersize', 10 )
  title ( 'matern: Covariance K(S,T) (dots), and Estimate from Realizations (mesh)' )
  print -dpng 'matern_figure5.png'
%
