...
%*****************************************************************************80
%
%% POWER_TEST examines the power correlation.
%
%  Licensing:
%
...
%
  F = fred ( @power_correlation, domain ( [ a, b ] ) );
%
...
%
  ...
  title ( 'power: Mercer eigenvalues' );
  xlabel ( '<--- N --->')
  grid on
  print -dpng 'power_figure1.png'
%
...
%
  ...
  title ( 'power: Mercer eigenfunction PSI(1)' )
  grid on
  subplot ( 4, 1, 2 )
  plot ( Psi(:,2), 'Linewidth', 2 );
  title ( 'power: Mercer eigenfunction PSI(2)' )
  grid on
  subplot ( 4, 1, 3 )
  plot ( Psi(:,3), 'Linewidth', 2 );
  title ( 'power: Mercer eigenfunction PSI(3)' )
  grid on
  if ( 4 <= eigen_num )
    subplot ( 4, 1, 4 )
    plot ( Psi(:,4), 'Linewidth', 2 );
    title ( 'power: Mercer eigenfunction PSI(4)' )
    grid on
  end
  print -dpng 'power_figure2.png'
%
...
%
  ...
  xlabel ( '<-- N -->' )
  title ( 'power: Eigenvalue decay rate' );
  print -dpng 'power_figure3.png'
%
...
%
  ...
  title ( 'power: 40 Random Realizations X(t,omega), and their Mean.' )
  print -dpng 'power_figure4.png'
%
...
%
  ...
  mesh ( S, T, C );
  hold on;
  D = power_correlation ( S, T );
  plot3 ( S, T, D, 'k.', 'markersize', 10 )
  title ( 'power: Covariance K(S,T) (dots), and Estimate from Realizations (mesh)' )
  print -dpng 'power_figure5.png'
%
