function pentaspherical_test ( )

%*****************************************************************************80
%
%% PENTASPHERICAL_TEST examines the pentaspherical correlation.
%
%  Discussion:
%
...
%
  ...
  timestamp ( );
  fprintf ( 1, '\n' );
  fprintf ( 1, 'PENTASPHERICAL_TEST:\n' );
  fprintf ( 1, '  Demonstrate Mercer''s theorem and the KL expansion\n' );
  fprintf ( 1, '  for the pentaspherical kernel.\n' );
%
%  Set the interval.
%
  a = 0.0;
...
%
  F = fred ( @pentaspherical_correlation, domain ( [ a, b ] ) );
%
...
%
  ...
  title ( 'pentaspherical: Mercer eigenvalues' );
  xlabel ( '<--- N --->')
  grid on
  print -dpng 'pentaspherical_figure1.png'
%
...
%
  ...
  title ( 'pentaspherical: Mercer eigenfunction PSI(1)' )
  grid on
  if ( 2 <= eigen_num )
    subplot ( 4, 1, 2 )
    plot ( Psi(:,2), 'Linewidth', 2 );
    title ( 'pentaspherical: Mercer eigenfunction PSI(2)' )
    grid on
  end
  if ( 3 <= eigen_num )
    subplot ( 4, 1, 3 )
    plot ( Psi(:,3), 'Linewidth', 2 );
    title ( 'pentaspherical: Mercer eigenfunction PSI(3)' )
    grid on
  end
  if ( 4 <= eigen_num )
    subplot ( 4, 1, 4 )
    plot ( Psi(:,4), 'Linewidth', 2 );
    title ( 'pentaspherical: Mercer eigenfunction PSI(4)' )
    grid on
  end
  print -dpng 'pentaspherical_figure2.png'
%
...
%
  ...
  xlabel ( '<-- N -->' )
  title ( 'pentaspherical: Eigenvalue decay rate' );
  print -dpng 'pentaspherical_figure3.png'
%
...
%
  ...
  title ( 'pentaspherical: 40 Random Realizations X(t,omega), and their Mean.' )
  print -dpng 'pentaspherical_figure4.png'
%
...
%
  ...
  mesh ( S, T, C );
  hold on;
  D = pentaspherical_correlation ( S, T );
  plot3 ( S, T, D, 'k.', 'markersize', 10 )
  title ( 'pentaspherical: Covariance K(S,T) (dots), and Estimate from Realizations (mesh)' )
  print -dpng 'pentaspherical_figure5.png'
%
