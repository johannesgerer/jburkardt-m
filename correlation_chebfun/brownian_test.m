...
%*****************************************************************************80
%
%% BROWNIAN_TEST examines the brownian correlation.
%
%  Discussion:
%
%    The correlation function rises sharply at S = 0.  This eigenvalue
%    computation seems to require starting some distance away from 0.
%    Here, we've set [A,B] to [0.1, 5.0].
%
...
%
  F = fred ( @brownian_correlation, domain ( [ a, b ] ) );
%
...
%
  ...
  title ( 'brownian: Mercer eigenvalues' );
  xlabel ( '<--- N --->')
  grid on
  print -dpng 'brownian_figure1.png'
%
...
%
  ...
  title ( 'brownian: Mercer eigenfunction PSI(1)' )
  grid on
  subplot ( 4, 1, 2 )
  plot ( Psi(:,2), 'Linewidth', 2 );
  title ( 'brownian: Mercer eigenfunction PSI(2)' )
  grid on
  subplot ( 4, 1, 3 )
  plot ( Psi(:,3), 'Linewidth', 2 );
  title ( 'brownian: Mercer eigenfunction PSI(3)' )
  grid on
  subplot ( 4, 1, 4 )
  plot ( Psi(:,4), 'Linewidth', 2 );
  title ( 'brownian: Mercer eigenfunction PSI(4)' )
  grid on
  print -dpng 'brownian_figure2.png'
%
...
%
  ...
  xlabel ( '<-- N -->' )
  title ( 'brownian: Eigenvalue decay rate' );
  print -dpng 'brownian_figure3.png'
%
...
%
  ...
  title ( 'brownian: 40 Random Realizations X(t,omega), and their Mean.' )
  print -dpng 'brownian_figure4.png'
%
...
%
  ...
  mesh ( S, T, C );
  hold on;
  D = brownian_correlation ( S, T );
  plot3 ( S, T, D, 'k.', 'markersize', 10 )
  title ( 'brownian: Covariance K(S,T) (dots), and Estimate from Realizations (mesh)' )
  print -dpng 'brownian_figure5.png'
%
...
%
  ...
  rhobar = 1.0;
  for i = 1 : 10
    K = @ ( s, t ) brownian ( 0, abs ( s - t ) / rhobar );
    F = fred ( K, domain ( [ a, b ] ) );
