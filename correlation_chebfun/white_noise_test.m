...
%*****************************************************************************80
%
%% WHITE_NOISE_TEST examines the white_noise correlation.
%
%  Discussion:
%
%    This code won't converge for ANY value of EIGEN_NUM.
%
...
%
  ...
  timestamp ( );
  fprintf ( 1, '\n' );
  fprintf ( 1, 'WHITE_NOISE_TEST:\n' );
  fprintf ( 1, '  Demonstrate Mercer''s theorem and the KL expansion\n' );
  fprintf ( 1, '  for the white_noise kernel.\n' );
%
%  Set the interval.
%
  a = 0.0;
...
%
  F = fred ( @white_noise_correlation, domain ( [ a, b ] ) );
%
...
%
  ...
  title ( 'white_noise: Mercer eigenvalues' );
  xlabel ( '<--- N --->')
  grid on
  print -dpng 'white_noise_figure1.png'
%
...
%
  ...
  title ( 'white_noise: Mercer eigenfunction PSI(1)' )
  grid on
  subplot ( 4, 1, 2 )
  plot ( Psi(:,2), 'Linewidth', 2 );
  title ( 'white_noise: Mercer eigenfunction PSI(2)' )
  grid on
  subplot ( 4, 1, 3 )
  plot ( Psi(:,5), 'Linewidth', 2 );
  title ( 'white_noise: Mercer eigenfunction PSI(5)' )
  grid on
  subplot ( 4, 1, 4 )
  plot ( Psi(:,10), 'Linewidth', 2 );
  title ( 'white_noise: Mercer eigenfunction PSI(10)' )
  grid on
  print -dpng 'white_noise_figure2.png'
%
...
%
  ...
  xlabel ( '<-- N -->' )
  title ( 'white_noise: Eigenvalue decay rate' );
  print -dpng 'white_noise_figure3.png'
%
...
%
  ...
  title ( 'white_noise: 40 Random Realizations X(t,omega), and their Mean.' )
  print -dpng 'white_noise_figure4.png'
%
...
%
  ...
  mesh ( S, T, C );
  hold on;
  D = white_noise_correlation ( S, T );
  plot3 ( S, T, D, 'k.', 'markersize', 10 )
  title ( 'white_noise: Covariance K(S,T) (dots), and Estimate from Realizations (mesh)' )
  print -dpng 'white_noise_figure5.png'
%
