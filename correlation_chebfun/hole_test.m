...
%*****************************************************************************80
%
%% HOLE_TEST examines the hole correlation.
%
%  Licensing:
%
...
%
  F = fred ( @hole_correlation, domain ( [ a, b ] ) );
%
...
%
  ...
  title ( 'hole: Mercer eigenvalues' );
  xlabel ( '<--- N --->')
  grid on
  print -dpng 'hole_figure1.png'
%
...
%
  ...
  title ( 'hole: Mercer eigenfunction PSI(1)' )
  grid on
  subplot ( 4, 1, 2 )
  plot ( Psi(:,2), 'Linewidth', 2 );
  title ( 'hole: Mercer eigenfunction PSI(2)' )
  grid on
  subplot ( 4, 1, 3 )
  plot ( Psi(:,3), 'Linewidth', 2 );
  title ( 'hole: Mercer eigenfunction PSI(3)' )
  grid on
  if ( 4 <= eigen_num )
    subplot ( 4, 1, 4 )
    plot ( Psi(:,4), 'Linewidth', 2 );
    title ( 'hole: Mercer eigenfunction PSI(4)' )
    grid on
  end
  print -dpng 'hole_figure2.png'
%
...
%
  ...
  xlabel ( '<-- N -->' )
  title ( 'hole: Eigenvalue decay rate' );
  print -dpng 'hole_figure3.png'
%
...
%
  ...
  title ( 'hole: 40 Random Realizations X(t,omega), and their Mean.' )
  print -dpng 'hole_figure4.png'
%
...
%
  ...
  mesh ( S, T, C );
  hold on;
  D = hole_correlation ( S, T );
  plot3 ( S, T, D, 'k.', 'markersize', 10 )
  title ( 'hole: Covariance K(S,T) (dots), and Estimate from Realizations (mesh)' )
  print -dpng 'hole_figure5.png'
%
