function damped_sine_test ( )

%*****************************************************************************80
%
%% DAMPED_SINE_TEST examines the damped_sine correlation.
%
%  Discussion:
%
...
%
  ...
  timestamp ( );
  fprintf ( 1, '\n' );
  fprintf ( 1, 'DAMPED_SINE_TEST:\n' );
  fprintf ( 1, '  Demonstrate Mercer''s theorem and the KL expansion\n' );
  fprintf ( 1, '  for the damped_sine kernel.\n' );
%
%  Set the interval.
%
  a = 0.0;
...
%
  F = fred ( @damped_sine_correlation, domain ( [ a, b ] ) );
%
...
%
  ...
  title ( 'damped_sine: Mercer eigenvalues' );
  xlabel ( '<--- N --->')
  grid on
  print -dpng 'damped_sine_figure1.png'
%
...
%
  ...
  title ( 'damped_sine: Mercer eigenfunction PSI(1)' )
  grid on
  if ( 2 <= eigen_found )
    subplot ( 4, 1, 2 )
    plot ( Psi(:,2), 'Linewidth', 2 );
    title ( 'damped_sine: Mercer eigenfunction PSI(2)' )
    grid on
  end
  if ( 3 <= eigen_found )
    subplot ( 4, 1, 3 )
    plot ( Psi(:,3), 'Linewidth', 2 );
    title ( 'damped_sine: Mercer eigenfunction PSI(3)' )
    grid on
  end
  if ( 4 <= eigen_found )
    subplot ( 4, 1, 4 )
    plot ( Psi(:,4), 'Linewidth', 2 );
    title ( 'damped_sine: Mercer eigenfunction PSI(4)' )
    grid on
  end
  print -dpng 'damped_sine_figure2.png'
%
...
%
  ...
  xlabel ( '<-- N -->' )
  title ( 'damped_sine: Eigenvalue decay rate' );
  print -dpng 'damped_sine_figure3.png'
%
...
%
  ...
  title ( 'damped_sine: 40 Random Realizations X(t,omega), and their Mean.' )
  print -dpng 'damped_sine_figure4.png'
%
...
%
  ...
  mesh ( S, T, C );
  hold on;
  D = damped_sine_correlation ( S, T );
  plot3 ( S, T, D, 'k.', 'markersize', 10 )
  title ( 'damped_sine: Covariance K(S,T) (dots), and Estimate from Realizations (mesh)' )
  print -dpng 'damped_sine_figure5.png'
%
...
%
  ...
  for i = 1 : 10
    ...
    F = fred ( K, domain ( [ a, b ] ) );
    lambda_vec = eigs ( F, eigen_found, 'lm' );
