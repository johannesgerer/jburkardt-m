...
%*****************************************************************************80
%
%% EXPONENTIAL_TEST examines the exponential correlation.
%
%  Discussion:
%
...
%
  ...
  timestamp ( );
  fprintf ( 1, '\n' );
  fprintf ( 1, 'EXPONENTIAL_TEST:\n' );
  fprintf ( 1, '  Demonstrate Mercer''s theorem and the KL expansion\n' );
  fprintf ( 1, '  for the exponential kernel.\n' );
%
%  Set the interval.
%
  a = 0.0;
...
%
  F = fred ( @exponential_correlation, domain ( [ a, b ] ) );
%
...
%
  ...
  title ( 'exponential: Mercer eigenvalues' );
  xlabel ( '<--- N --->')
  grid on
  print -dpng 'exponential_figure1.png'
%
...
%
  ...
  title ( 'exponential: Mercer eigenfunction PSI(1)' )
  grid on
  subplot ( 4, 1, 2 )
  plot ( Psi(:,2), 'Linewidth', 2 );
  title ( 'exponential: Mercer eigenfunction PSI(2)' )
  grid on
  subplot ( 4, 1, 3 )
  plot ( Psi(:,5), 'Linewidth', 2 );
  title ( 'exponential: Mercer eigenfunction PSI(5)' )
  grid on
  subplot ( 4, 1, 4 )
  plot ( Psi(:,10), 'Linewidth', 2 );
  title ( 'exponential: Mercer eigenfunction PSI(10)' )
  grid on
  print -dpng 'exponential_figure2.png'
%
...
%  For the exponential kernel, the decay rate is 1/N^2.
%
  ...
  plot ( [0,eigen_num], [-2,-2], 'k-', 'Linewidth', 2 );
  xlabel ( '<-- N -->' )
  title ( 'exponential: Eigenvalue decay rate tends to n^(-2)' );
  print -dpng 'exponential_figure3.png'
%
...
%
  ...
  title ( 'exponential: 40 Random Realizations X(t,omega), and their Mean.' )
  print -dpng 'exponential_figure4.png'
%
...
%
  ...
  mesh ( S, T, C );
  hold on;
  D = exponential_correlation ( S, T );
  plot3 ( S, T, D, 'k.', 'markersize', 10 )
  title ( 'exponential: Covariance K(S,T) (dots), and Estimate from Realizations (mesh)' )
  print -dpng 'exponential_figure5.png'
%
