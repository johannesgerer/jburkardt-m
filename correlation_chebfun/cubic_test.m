...
%*****************************************************************************80
%
%% CUBIC_TEST examines the cubic correlation.
%
%  Discussion:
%
...
%
  F = fred ( @cubic_correlation, domain ( [ a, b ] ) );
%
...
%
  ...
  title ( 'cubic: Mercer eigenvalues' );
  xlabel ( '<--- N --->')
  grid on
  print -dpng 'cubic_figure1.png'
%
...
%
  ...
  title ( 'cubic: Mercer eigenfunction PSI(1)' )
  grid on
  subplot ( 4, 1, 2 )
  plot ( Psi(:,2), 'Linewidth', 2 );
  title ( 'cubic: Mercer eigenfunction PSI(2)' )
  grid on
  subplot ( 4, 1, 3 )
  plot ( Psi(:,5), 'Linewidth', 2 );
  title ( 'cubic: Mercer eigenfunction PSI(5)' )
  grid on
  subplot ( 4, 1, 4 )
  plot ( Psi(:,10), 'Linewidth', 2 );
  title ( 'cubic: Mercer eigenfunction PSI(10)' )
  grid on
  print -dpng 'cubic_figure2.png'
%
...
%
  ...
  xlabel ( '<-- N -->' )
  title ( 'cubic: Eigenvalue decay rate' );
  print -dpng 'cubic_figure3.png'
%
...
%
  ...
  title ( 'cubic: 40 Random Realizations X(t,omega), and their Mean.' )
  print -dpng 'cubic_figure4.png'
%
...
%
  ...
  mesh ( S, T, C );
  hold on;
  D = cubic_correlation ( S, T );
  plot3 ( S, T, D, 'k.', 'markersize', 10 )
  title ( 'cubic: Covariance K(S,T) (dots), and Estimate from Realizations (mesh)' )
  print -dpng 'cubic_figure5.png'
%
