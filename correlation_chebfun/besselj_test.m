function besselj_test ( )

%*****************************************************************************80
%
%% BESSELJ_CORRELATION_TEST examines the besselj correlation.
%
%  Discussion:
%
...
%
  F = fred ( @besselj_correlation, domain ( [ a, b ] ) );
%
...
%
  ...
  title ( 'besselj: Mercer eigenvalues' );
  xlabel ( '<--- N --->')
  grid on
  print -dpng 'besselj_figure1.png'
%
...
%
  ...
  title ( 'besselj: Mercer eigenfunction PSI(1)' )
  grid on
  subplot ( 4, 1, 2 )
  plot ( Psi(:,2), 'Linewidth', 2 );
  title ( 'besselj: Mercer eigenfunction PSI(2)' )
  grid on
  subplot ( 4, 1, 3 )
  plot ( Psi(:,3), 'Linewidth', 2 );
  title ( 'besselj: Mercer eigenfunction PSI(3)' )
  grid on
  subplot ( 4, 1, 4 )
  plot ( Psi(:,4), 'Linewidth', 2 );
  title ( 'besselj: Mercer eigenfunction PSI(4)' )
  grid on
  print -dpng 'besselj_figure2.png'
%
...
%
  ...
  xlabel ( '<-- N -->' )
  title ( 'besselj: Eigenvalue decay rate' );
  print -dpng 'besselj_figure3.png'
%
...
%
  ...
  title ( 'besselj: 40 Random Realizations X(t,omega), and their Mean.' )
  print -dpng 'besselj_figure4.png'
%
...
%
  ...
  mesh ( S, T, C );
  hold on;
  D = besselj_correlation ( S, T );
  plot3 ( S, T, D, 'k.', 'markersize', 10 )
  title ( 'besselj: Covariance K(S,T) (dots), and Estimate from Realizations (mesh)' )
  print -dpng 'besselj_figure5.png'
%
...
%
  ...
  rhobar = 1.0;
  for i = 1 : 10
    K = @ ( s, t ) besselj ( 0, abs ( s - t ) / rhobar );
    F = fred ( K, domain ( [ a, b ] ) );
