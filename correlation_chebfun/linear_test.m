...
%*****************************************************************************80
%
%% LINEAR_TEST examines the linear correlation.
%
%  Discussion:
%
...
%
  F = fred ( @linear_correlation, domain ( [ a, b ] ) );
%
...
%
  ...
  title ( 'linear: Mercer eigenvalues' );
  xlabel ( '<--- N --->')
  grid on
  print -dpng 'linear_figure1.png'
%
...
%
  ...
  title ( 'linear: Mercer eigenfunction PSI(1)' )
  grid on
  subplot ( 4, 1, 2 )
  plot ( Psi(:,2), 'Linewidth', 2 );
  title ( 'linear: Mercer eigenfunction PSI(2)' )
  grid on
  subplot ( 4, 1, 3 )
  plot ( Psi(:,3), 'Linewidth', 2 );
  title ( 'linear: Mercer eigenfunction PSI(3)' )
  grid on
  if ( 4 <= eigen_num )
    subplot ( 4, 1, 4 )
    plot ( Psi(:,4), 'Linewidth', 2 );
    title ( 'linear: Mercer eigenfunction PSI(4)' )
    grid on
  end
  print -dpng 'linear_figure2.png'
%
...
%
  ...
  xlabel ( '<-- N -->' )
  title ( 'linear: Eigenvalue decay rate' );
  print -dpng 'linear_figure3.png'
%
...
%
  ...
  title ( 'linear: 40 Random Realizations X(t,omega), and their Mean.' )
  print -dpng 'linear_figure4.png'
%
...
%
  ...
  mesh ( S, T, C );
  hold on;
  D = linear_correlation ( S, T );
  plot3 ( S, T, D, 'k.', 'markersize', 10 )
  title ( 'linear: Covariance K(S,T) (dots), and Estimate from Realizations (mesh)' )
  print -dpng 'linear_figure5.png'
%
