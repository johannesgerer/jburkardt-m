function int_exactness_jacobi ( quad_filename, degree_max, alpha, beta )

%*****************************************************************************80
%
%% MAIN is the main program for INT_EXACTNESS_JACOBI.
%
%  Discussion:
%
...
%
  ...
  fprintf ( 1, '\n' );
  fprintf ( 1, '  Investigate the polynomial exactness of a Gauss-Jacobi\n' );
...
%
  if ( 3 <= nargin )
    ...
    fprintf ( 1, '\n' );
    fprintf ( 1, 'INT_EXACTNESS_JACOBI:\n' );
    fprintf ( 1, '  ALPHA is the power of (1-X) in the weighting function.\n' );
    fprintf ( 1, '\n' );
...
%
  if ( 4 <= nargin )
    ...
    fprintf ( 1, '\n' );
    fprintf ( 1, 'INT_EXACTNESS_JACOBI:\n' );
    fprintf ( 1, '  BETA is the power of (1+X) in the weighting function.\n' );
    fprintf ( 1, '\n' );
    fprintf ( 1, '  BETA is a real number greater than -1.0.\n' );
    fprintf ( 1, '\n' );
...
%
  fprintf ( 1, '\n' );
  fprintf ( 1, 'INT_EXACTNESS_JACOBI: User input:\n' );
  ...
  fprintf ( 1, '  Exponent of (1-x), ALPHA = %f\n', alpha );
  fprintf ( 1, '  Exponent of (1+x), BETA  = %f\n', beta );
%
%  Read the X file.
%
...
%
  ...
  fprintf ( 1, '  a Gauss-Jacobi rule\n' );
  fprintf ( 1, '  ORDER = %d\n', order );
  fprintf ( 1, '  ALPHA = %f\n', alpha );
  fprintf ( 1, '  BETA  = %f\n', beta );
  fprintf ( 1, '\n' );
  fprintf ( 1, '  Standard rule:\n' );
  fprintf ( 1, '    Integral ( -1 <= x <= +1 ) (1-x)^alpha (1+x)^beta f(x) dx\n' );
...
%
  fprintf ( 1, '\n' );
  fprintf ( 1, '  A Gauss-Jacobi rule would be able to exactly\n' );
  ...
  for degree = 0 : degree_max
    ...
    quad_error = monomial_quadrature_jacobi ( degree, alpha, beta, order, w, x );
...
function quad_error = monomial_quadrature_jacobi ( expon, alpha, beta, ...
  order, w, x )
...
%
  exact = jacobi_integral ( expon, alpha, beta );
%
