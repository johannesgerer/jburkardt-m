...
%
%    This program reads these files and creates a MEDIT file, whose
...
%
  ...
  fprintf ( 1, '\n' );
  fprintf ( 1, 'FEM_TO_MEDIT\n' );
  fprintf ( 1, '  MATLAB version.\n' );
  fprintf ( 1, '  Read a set of FEM files;\n' );
...
%
  ...
  mesh_filename               = strcat ( prefix, '.mesh' );
%
%  Now we know what to do.
%
  fem_to_medit_handle ( node_filename, element_filename, ...
...
function fem_to_medit_handle ( node_filename, element_filename, ...
  boundary_node_mask_filename, mesh_filename )

%*****************************************************************************80
%
%% FEM_TO_MEDIT_HANDLE copies mesh data from a FEM dataset to a MEDIT data set.
%
%  Licensing:
%
%    This code is distributed under the GNU LGPL license.
...
%  Parameters:
%
%    Input, string NODE_FILENAME, the FEM node filename.
%
%    Input, string ELEMENT_FILENAME, the FEM element filename.
%
%    Input, string BOUNDARY_NODE_MASK_FILENAME, the FEM
%    boundary node mask filename.
%
%    Input, string MESH_FILENAME, the MEDIT mesh filename.
%
  fprintf ( 1, '\n' );
  fprintf ( 1, '  Reading FEM files:\n' );
  fprintf ( 1, '    "%s".\n', node_filename );
  fprintf ( 1, '    "%s".\n', element_filename );
  fprintf ( 1, '    "%s".\n', boundary_node_mask_filename );

  [ dim_num, node_num, element_num, element_order ] = fem_header_read ( ...
    node_filename, element_filename );

  fem_header_print ( dim_num, node_num, element_num, element_order );
%
%  Allocate space for the data, and read the data.
%
  [ node_coord, element_node, boundary_node_mask ] = fem_data_read ( ...
    node_filename, element_filename, boundary_node_mask_filename, dim_num, ...
    node_num, element_num, element_order );
%
...
%
  ...
  tetrahedron_label(1,1:tetrahedrons) = 0;

  vertex_coordinate(1:dim,1:vertices) = node_coord(1:dim,1:vertices);
  vertex_label(1,1:vertices) = boundary_node_mask(1,1:vertices);
%
