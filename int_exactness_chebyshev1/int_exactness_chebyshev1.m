function int_exactness_chebyshev1 ( quad_filename, degree_max )
...
%  Discussion:
%
%    This program investigates a standard Gauss-Chebvyshev type 1 quadrature rule
...
%
  ...
  fprintf ( 1, '\n' );
  fprintf ( 1, '  Investigate the polynomial exactness of a Gauss-Chebyshev1\n' );
  fprintf ( 1, '  type 1 quadrature rule by integrating all monomials up to a given\n' );
...
%
  fprintf ( 1, '\n' );
  fprintf ( 1, 'INT_EXACTNESS_CHEBYSHEV1: User input:\n' );
...
%
  ...
  fprintf ( 1, '  a Gauss-Chebyshev type 1 rule\n' );
  fprintf ( 1, '  ORDER = %d\n', order );
  fprintf ( 1, '\n' );
  fprintf ( 1, '  Standard rule:\n' );
  fprintf ( 1, '    Integral ( -1 <= x <= +1 ) f(x) / ( 1 - x^2 ) dx\n' );
...
%
  fprintf ( 1, '\n' );
  fprintf ( 1, '  A Gauss-Chebyshev type 1 rule would be able to exactly\n' );
  ...
  for degree = 0 : degree_max
    ...
    quad_error = monomial_quadrature_chebyshev1 ( degree, order, w, x );
...
function quad_error = monomial_quadrature_chebyshev1 ( expon, order, w, x )

%*****************************************************************************80
%
%% MONOMIAL_QUADRATURE_CHEBYSHEV1 applies a quadrature rule to a monomial.
%
%  Licensing:
%
%    This code is distributed under the GNU LGPL license.
...
%  Get the exact value of the integral.
%
  exact = chebyshev1_integral ( expon );
%
