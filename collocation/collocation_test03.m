function collocation_test03 ( n )

%*****************************************************************************80
%
%% COLLOCATION_TEST03 collocates a two point boundary value problem.
%
%  Discussion:
%
...
%    with X(1) = 0.0 and X(N) = 2.0, and assuming G(X) is a sum of
%    sine functions.
%
%  Licensing:
%
...
%
  ...
  fprintf ( 1, '  Let G(X) be C1+C2*X+C(3:N)*SIN((1:N-2)*PI*X/(N-2)).\n' );
...
%
  ...
  for i = 2 : n1 - 1
    for j = 3 : n1
      A(i,j) = A(i,j) + ( j - 2 )^2 * pi^2 * sin ( ( j - 2 ) ...
        * pi * x1(i) / ( n1 - 2 ) ) / ( n1 - 2 ) ^ 2;
    end
  end
...
%
  y1(1:n1) = 0.0;
  for i = 1 : n1
    y1(i) = y1(i) + c(1) + c(2) * x1(i);
    for j = 3 : n1
      y1(i) = y1(i) + c(j) * sin ( ( j - 2 ) * pi * x1(i) / ( n1 - 2 ) );
    end
  end
%
...
%
  ...
  y2(1:n2) = 0.0;
  for i = 1 : n2
    y2(i) = y2(i) + c(1) + c(2) * x2(i);
    for j = 3 : n1
      y2(i) = y2(i) + c(j) * sin ( ( j - 2 ) * pi * x2(i) / ( n1 - 2 ) );
    end
  end
  ...
  grid on
  title ( 'Collocate two point BVP using sines.', 'FontSize', 24 )
  hold off
%
%  Save plot in a file.
%
