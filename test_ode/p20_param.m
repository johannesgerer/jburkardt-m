function value = p20_param ( action, name, value )

%*****************************************************************************80
%
%% P20_PARAM defaults, sets, or gets the parameters for problem 20.
%
%  Discussion:
%
...
%      value = p20_param ( 'GET', 'DELTA' )
%    or set to a new value, as in:
%      p20_param ( 'SET', 'DELTA', 7 )
%
...
%
  ...
  elseif ( s_eqi ( action, 'SET' ) )
    ...
    if ( s_eqi ( name, 'DELTA' ) )
      ...
      fprintf ( 1, '\n' );
      fprintf ( 1, 'P20_PARAM - Fatal error!\n' );
      fprintf ( 1, '  Unrecognized variable name: "%s".\n', name );
      error ( 'P20_PARAM - Fatal error!' );
    end
