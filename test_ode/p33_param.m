function value = p33_param ( action, name, value )

%*****************************************************************************80
%
%% P33_PARAM defaults, sets, or gets the parameters for problem 33.
%
%  Discussion:
%
...
%      value = p33_param ( 'GET', 'DELTA' )
%    or set to a new value, as in:
%      p33_param ( 'SET', 'DELTA', 7 )
%
...
%
  ...
  elseif ( s_eqi ( action, 'SET' ) )
    ...
    if ( s_eqi ( name, 'DELTA' ) )
      ...
      fprintf ( 1, '\n' );
      fprintf ( 1, 'P33_PARAM - Fatal error!\n' );
      fprintf ( 1, '  Unrecognized variable name: "%s".\n', name );
      error ( 'P33_PARAM - Fatal error!' );
    end
