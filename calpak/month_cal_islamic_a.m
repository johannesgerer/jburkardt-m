function month_cal_islamic_a ( y, m )
...
%
  ...
  w = ymdf_to_weekday_islamic_a ( y2, m2, d, f );
%
...
%  Print out a heading.
%
  s1 = month_to_month_name_islamic ( m2 );
  s2 = y_to_s_islamic ( y );

  fprintf ( 1, '\n' );
  fprintf ( 1, 'ISLAMIC-A CALENDAR\n' );
  fprintf ( 1, '%s %s\n', s1, s2 );
  fprintf ( 1, '\n' );
%
...
%
  while ( iday <= month_length_islamic ( y2, m2 ) )
    ...
    for w = 1 : 7
      ...
      if ( iday < 1 )
        fprintf ( 1, '   ' );
      elseif ( month_length_islamic ( y2, m2 ) < iday )
        fprintf ( 1, '   ' );
      else
        fprintf ( 1, ' %2d', iday );
      end
