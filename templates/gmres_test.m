...
%*****************************************************************************80
%
%% GMRES_TEST tests GMRES.
% 
  fprintf ( 1, '\n' );
  fprintf ( 1, 'GMRES_TEST:\n' );
  fprintf ( 1, '  GMRES uses the Generalized Minimum Residual \n' );
  ...
  fprintf ( 1, '\n' );
  fprintf ( 1, '  The method is restarted after every %d steps;\n', restart );
  fprintf ( 1, '  The maximum number of steps is %d.\n', max_it );
  fprintf ( 1, '  The error tolerance is %f\n', tol );

  [ x, error_norm, iter, flag ] = gmres ( A, x, b, M, restart, max_it, tol, n );

  fprintf ( 1, '\n' );
  fprintf ( 1, '  The GMRES routine has returned with FLAG = %d\n', flag );
  if ( flag == 0 )
