function vector_arrows_grid ( node_file, velocity_file, scale, number )

%*****************************************************************************80
%
%% VELOCITY_ARROWS_GRID plots a velocity field grid based on scattered data.
%
%  Discussion:
%
...
%
  ...
  if ( dim_num ~= 2 )
    ...
    error ( 'VELOCITY_ARROWS_GRID - Fatal error!' );
  end
...
%
  ...
  u_grid = griddata ( node_xy(1,:), node_xy(2,:), uv(1,:), x_grid, y_grid, 'cubic' );
  v_grid = griddata ( node_xy(1,:), node_xy(2,:), uv(2,:), x_grid, y_grid, 'cubic' );
%
...
%
  ...
  title ( 'Velocity Vectors', 'FontName', 'Helvetica', 'FontWeight', 'bold', ...
    'FontSize', 16 );
