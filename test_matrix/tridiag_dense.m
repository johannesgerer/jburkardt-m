function T = tridiag_dense ( n, x, y, z )

%%TRIDIAG_DENSE  Tridiagonal matrix
%
%         TRIDIAG_DENSE(X, Y, Z) is the tridiagonal matrix with subdiagonal X,
%         diagonal Y, and superdiagonal Z.
%         X and Z must be vectors of dimension one less than Y.
%
%         Alternatively TRIDIAG_DENSE(N, C, D, E), where C, D, and E are all
...
%
%         TRIDIAG_DENSE(N) is the same as TRIDIAG_DENSE(N,-1,2,-1), which is
...
if max( [ size(x) size(y) size(z) ] ) == 1
   ...
   if ( ny - nx - 1) | ( ny - nz -1 )
      ...
      ny
      nz
      error('TRIDIAG_DENSE: Dimensions of vector arguments are incorrect.')
   end
end
