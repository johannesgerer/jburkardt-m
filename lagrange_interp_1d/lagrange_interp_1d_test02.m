function lagrange_interp_1d_test02 ( prob, nd )

%*****************************************************************************80
%
%% LAGRANGE_INTERP_1D_TEST02 tests LAGRANGE_VALUE_1D with evenly spaced data
%
%  Licensing:
%
...
%
  fprintf ( 1, '\n' );
  fprintf ( 1, 'LAGRANGE_INTERP_1D_TEST02:\n' );
  fprintf ( 1, '  Interpolate data from TEST_INTERP_1D problem #%d.\n', prob );
  fprintf ( 1, '  Use even spacing for data points.\n' );
...
%
  ...
  xlabel ( '<--- X --->' );
  ylabel ( '<--- Y --->' );
  title ( sprintf ( 'Interpolation data at %d even nodes', nd ) )
  grid on
  hold off
  filename = sprintf ( 'p%02d_dataeven_%02d.png', prob, nd );
  print ( '-dpng', filename );
  fprintf ( 1, '\n' );
...
%
  ...
  xlabel ( '<--- X --->' );
  ylabel ( '<--- Y --->' );
  title ( sprintf ( 'Lagrange/Even Polynomial Interpolant for %d nodes', nd ) )
  grid on
  hold off

  filename = sprintf ( 'p%02d_lageven_%02d.png', prob, nd );
  print ( '-dpng', filename );
  fprintf ( 1, '  Created plot file "%s".\n', filename );
