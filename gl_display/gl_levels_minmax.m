function [ grid_level, grid_order, grid_point ] = gl_levels_minmax ( ...
  dim_num, level_min, level_max, grid_num, point_num )

%*****************************************************************************80
%
%% GL_LEVELS_MINMAX computes GL grids with LEVEL_MIN <= LEVEL <= LEVEL_MAX.
%
%  Discussion:
%
%    The GL grids are required to have an order that is 2**LEVEL + 1.
%
%    The necessary dimensions of GRID_LEVEL, GRID_ORDER and GRID_POINT can be
%    determined by calling GL_LEVELS_MINMAX first.
%
%  Licensing:
%
...
%    grid, the level of the Gauss Legendre rule in each dimension.
%
%    Output, integer GRID_ORDER(DIM_NUM,GRID_NUM), contains, for each
%    grid, the order of the Gauss Legendre rule in each dimension.
%
...
%
      order_1d = gl_level_to_order ( dim_num, level_1d );
