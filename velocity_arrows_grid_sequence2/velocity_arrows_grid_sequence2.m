function velocity_arrows_grid_sequence2 ( xyuv_filename, scale, number )

%*****************************************************************************80
%
%% VELOCITY_ARROWS_GRID_SEQUENCE2 creates a sequence of velocity plots.
%
%  Discussion:
%
...
%
%    The "VELOCITY_ARROWS" part of the name indicates that the program
%    is used to draw arrows representing velocities at selected points in
%    a region.
%
%    The "GRID" part of the name reflects the fact that the program is
%    intended to replace messy input data (scattered data, data that
%    is not very dense, data that is too dense) by an interpolated
%    function which is then sampled on a regular grid.
%
%    The "SEQUENCE2" part of the name indicates that the program will
%    process a sequence of data files, creating a separate image of each
%    one, AND that the node and velocity data for a single plot is 
%    included in single "XYUV" file.  
%
%    The program can handle many files, with 'consecutive' names, such
%    as "fred01.txt", "fred02.txt", "fred03.txt" and so on.  The user only
%    has to tell the program to process the first file, and it will 
%    automatically continue to process files until it reaches the end of the 
%    sequence of names.
%
%    The input files:
%
%    Each XYUV input data file is assumed to store the value of the 
%    X, Y coordinates and U, V (horizontal and vertical) velocity
%    components at a point, one set of values per line.
%
%    Internal processing:
%
%    Once the first file is processed, the routine checks to see if there
%    is a second "consecutive" file.  If so, that file is also processed, and 
%    so on, until presumably all files in the sequence have been handled.
%
%  Usage:
%
%    velocity_arrows_grid_sequence2 ( xyuv_filename, scale, number )
%
%    Note that a literal filename must be enclosed in single quotes.
%    Thus, a typical invocation might be
%
%      velocity_arrows_grid_sequence2 ( 'xyuv0001.txt', 1.0, 20 )
%
%    But if you simply say 
%
%      velocity_arrows_grid_sequence2
%
...
%
  timestamp ( );
  fprintf ( 1, '\n' );
  fprintf ( 1, 'VELOCITY_ARROWS_GRID_SEQUENCE2:\n' );
  fprintf ( 1, '  Given a sequence of XYUV files,\n' );
  fprintf ( 1, '  interpolate the data,\n' );
  fprintf ( 1, '  sample it on a regular rectangular grid,\n' );
  fprintf ( 1, '  display the velocity field,\n' );
  fprintf ( 1, '  and save each image in a PNG file.\n' );
%
%  Argument #1 is the XYUV file name.
%
  if ( nargin < 1 )
  ...
  if ( ~file_exist ( xyuv_filename ) )
    fprintf ( 1, '\n' );
    fprintf ( 1, 'VELOCITY_ARROWS_GRID_SEQUENCE2 - Fatal error!\n' );
    fprintf ( 1, '  The file "%s" does not exist.\n', xyuv_filename );
    return
  end
%
%  Argument #2 is the scale factor.
%
  if ( nargin < 2 )
    fprintf ( 1, '\n' );
    scale = input ( '  Enter the scale factor (0 for default scaling):' );
  end
%
%  Argument #3 is the number of grid points.
%
  if ( nargin < 3 )
    fprintf ( 1, '\n' );
...
%
%  Set the name of the first PNG file.
%
...
%  Determine the U, V maximum magnitude.
%
%  A single minimum / maximum is used for all the files, so that
%  they are properly scaled for animation.
%
  x_min = inf;
  x_max = -inf;
  y_min = inf;
  ...
  if ( mag_max == 0.0 )
    ...
    error ( 'VELOCITY_ARROWS_GRID_SEQUENCE2 - The input data is zero!' );
  end
...
%
  if ( number <= 1 )
    ...
    y_vec = 0.5 * ( y_min + y_max );

  elseif ( y_max - y_min < x_max - x_min ) then

    x_num = number;

    inc = ( x_max - x_min ) / ( x_num - 1 );

    y_num = ( round ( y_max - y_min ) / inc );
    y_frac = 0.5 * ( ( y_max - y_min ) - y_num * inc );

    x_vec = x_min          : inc : x_max;
    y_vec = y_min + y_frac : inc : y_max - y_frac;

  else

    y_num = number;

    inc = ( y_max - y_min ) / ( y_num - 1 );

    x_num = ( round ( x_max - x_min ) / inc );
    x_frac = 0.5 * ( ( x_max - x_min ) - x_num * inc );

    x_vec = x_min + x_frac : inc : x_max - x_frac;
    y_vec = y_min          : inc : y_max;
...
%
    u_grid = griddata ( xyuv(:,1), xyuv(:,2), xyuv(:,3), ...
      x_grid, y_grid, 'cubic' );

    v_grid = griddata ( xyuv(:,1), xyuv(:,2), xyuv(:,4), ...
      x_grid, y_grid, 'cubic' );
%
%  Make a vector plot using the grid data.
%
    h = quiver ( x_grid, y_grid, u_grid, v_grid, scale );
%
%  You can change the width of the arrows here.
%  You could also change their color.
%
    set ( h, 'LineWidth', 1 )
%
%  Provide text labels for the plot and the axes.
%
%   xlabel ( 'X', ...
%     'FontName', 'Helvetica', ...
%     'FontWeight', 'bold', ...
...
%     'FontSize', 16 );
%
%  For some reason, if you let MATLAB choose the axes, it's too generous.
%
    axis ( [ x_min, x_max, y_min, y_max ] );
    axis square
%
%  To turn off the axis labeling, tick marks and background,
%  uncomment the command "AXIS OFF"
%
%   axis off

    hold on
%
%  Plot the boundary in red.
%
    k = convhull ( xyuv(:,1), xyuv(:,2) );
    plot ( xyuv(k,1), xyuv(k,2), 'r' );
    hold on
%
%  We need to do this to add a small blank margin around the data.
%
