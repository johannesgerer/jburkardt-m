function quad_unit_quad_test ( degree_max )
...
%
  ...
  fprintf ( 1, '\n' );
  fprintf ( 1, 'QUAD_UNIT_QUAD_TEST\n' );
  fprintf ( 1, '  For the unit quadrilateral,\n' );
  fprintf ( 1, '  we approximate monomial integrals with\n' );
  fprintf ( 1, '    QUAD_UNIT_RULE, which returns M by N point rules..\n' );
  ...
  while ( 1 )
    ...
    for k = 1 : 5
      ...
      [ w, xy ] = quad_unit_rule ( order_1d );
      v = monomial_value ( dim_num, order, expon, xy );
      quad = quad_unit_volume ( ) *  ( w(1:order) * v(1:order)' );
