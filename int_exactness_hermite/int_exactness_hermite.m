function int_exactness_hermite ( quad_filename, degree_max, option )
...
%
  timestamp ( );
  fprintf ( 1, '\n' );
  fprintf ( 1, 'INT_EXACTNESS_HERMITE\n' );
...
%
  if ( 3 <= nargin )
    ...
    fprintf ( 1, '\n' );
    fprintf ( 1, '    0, the unweighted rule for:\n' );
    fprintf ( 1, '      Integral ( -oo < x < +oo )             f(x) dx\n' );
    fprintf ( 1, '\n' );
    fprintf ( 1, '    1, the physicist weighted rule for:\n' );
    fprintf ( 1, '      Integral ( -oo < x < +oo ) exp(-x*x)   f(x) dx\n' );
    fprintf ( 1, '\n' );
    fprintf ( 1, '    2, the probabilist weighted rule for:\n' );
    fprintf ( 1, '      Integral ( -oo < x < +oo ) exp(-x*x/2) f(x) dx\n' );
    fprintf ( 1, '\n' );
  ...
  if ( option < 0 || 2 < option )
    fprintf ( 1, '\n' );
    fprintf ( 1, 'INT_EXACTNESS_HERMITE - Fatal error!\n' );
    fprintf ( 1, '  Illegal value of OPTION.\n' );
    error ( 'INT_EXACTNESS_HERMITE - Fatal error!' );
  end
%
%  Summarize the input.
%
  fprintf ( 1, '\n' );
  fprintf ( 1, 'INT_EXACTNESS_HERMITE: User input:\n' );
...
%
  ...
  fprintf ( 1, '  a Gauss-Hermite rule\n' );
  fprintf ( 1, '  ORDER = %d\n', order );
  fprintf ( 1, '\n' );
  if ( option == 0 )
...
%
  ...
  for degree = 0 : degree_max
    ...
    quad_error = monomial_quadrature_hermite ( degree, order, option, w, x );
...
function quad_error = monomial_quadrature_hermite ( expon, order, option, w, x )
